...
# image1 = Image.open('./Flower/hoa cúc châu phi/hoa cúc châu phi04.png')
image1 = Image.open('./Flower/test/hoa cúc châu phi.png')
# image1 = Image.open('./test/test/hoa hồng.png')
...
# color_distance = normalize(color_distance)
# hog_distance = normalize(hog_distance)

combine_distance = [(color_distance[i], hog_distance[i], color_distance[i] +  hog_distance[i]) for i in range(len(color_distance))]
combine_distance.sort(key=lambda x: x[2])
for i in range(5):
...
color_distance.sort()
hog_distance.sort()
print(color_distance[:5])
print(hog_distance[:5])